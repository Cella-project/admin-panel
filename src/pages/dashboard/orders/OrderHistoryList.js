...
const OrderHistoryList = () => {
    ...
    if (orderHistoryCards !== null && orderHistoryCards.length > 0) {
        ...
        cards = [
            { title: 'Total Orders', content: orderHistoryCards.length, icon: "bi bi-people mint-green" },
            { title: 'Delivered', content: orderHistoryCards.filter(orderHistoryCard => orderHistoryCard.status === 'Delivered').length, icon: "bi bi-people mint-green" },
            { title: 'Pending', content: orderHistoryCards.filter(orderHistoryCard => orderHistoryCard.status === 'Pending').length, icon: "bi bi-people mint-green" },
            { title: 'Cancelled By Customer', content: orderHistoryCards.filter(orderHistoryCard => orderHistoryCard.status === 'CanceledByCustomer').length, icon: "bi bi-people mint-green" },
            { title: 'Cancelled By Admin', content: orderHistoryCards.filter(orderHistoryCard => orderHistoryCard.status === 'CanceledByAdmin').length, icon: "bi bi-people mint-green" },
        ]
    }
