...
const SubCategoryCard = ({ category }) => {
    ...
    const handleChangeState = async () => {
        dispatch(subCategoryActions.changeSubCategoryState(category._id));
    };
    ...
    return (
        <div className='category-card lists-card--info gray shadow-5px size-16px margin-10px-V inter radius-15px white-bg full-width flex-row-left-start2col'>
            {popupShown &&
                <Popup popupToggle={setPopupShown} header={'Edit Sub Category'} data={category} />
            }
            ...
            <div className='flex-col-center category-card--info padding-10px-H'>
                ...
                {!editMode && <div className='flex-row-right-start full-width' >
                    <Link to={`/admin-panel/products?category=${category._id}`} className="flex-row-right-start margin-6px-V size-12px pointer lists-card--link category-card--info--child">
                        Products
