...
import './AddSpecialtyMaterial.scss'
const AddSpecialtyMaterialForm = ({ popupToggle,data }) => {
    ...
    const formSubmissionHandler = (e) => {
        e.preventDefault();
        dispatch(specialityControlActions.addMaterial({ "title": enteredTitle ,'speciality':data}, () => {
            resetTitleInput();
            popupToggle(false);
    ...
    return (
        <form noValidate className="add-speciality-material" onSubmit={formSubmissionHandler}>
            <div className="full-width flex-col-left-start add-speciality-material--input-container">
                <label className="pointer full-width text-shadow gray font-bold margin-6px-V" htmlFor="title" >
                    Title
                </label>
                <div className={`full-width gray radius-10px white-bg flex-row-left-start add-speciality-material--input ${titleClasses}`}>
            ...
            <div className="add-speciality-material--actions flex-row-between full-width">
                ...
                <button
                    className="add-speciality-material--actions--button pointer radius-10px shadow-4px white text-shadow size-18px gray-bg"
                    onClick={() => {
                        resetTitleInput();
                        popupToggle(false);
                        document.getElementById("dashboard-view").style.zIndex = 10;
...
export default AddSpecialtyMaterialForm
