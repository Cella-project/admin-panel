...
const AddPaymentForm = ({ popupToggle }) => {
    ...
    const formSubmissionHandler = (e) => {
        ...
        dispatch(paymentActions.addPayment({ title: enteredTitle }, () => {
            resetTitleInput();
            popupToggle(false);
    ...
    return (
        <form noValidate className="add-payment" onSubmit={formSubmissionHandler}>
            ...
            <div className="add-payment--actions flex-row-between full-width">
                <button
                    className={`add-payment--actions--button pointer radius-10px shadow-4px ${mode === 'dark-mode' ? 'gray' : 'white'} text-shadow size-18px font-bold mint-green-bg`}
                    type="submit"
                >
                    Confirm
                </button>
